...
- **Alipay inbound owns a strict sequence** (`alipay-inbound.ts`): challenge → verify → atomic replay claim → handler → buffer + size check → confirm fulfillment → complete claim → release bytes. The buyer never receives the resource before fulfillment is confirmed. On confirmation failure the claim is deliberately **not** released — that is uncertain payment state needing reconciliation, not a retry.
