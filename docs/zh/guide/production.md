...
Alpha 是 Node-only SDK。请在服务端 Node.js runtime 中运行，Node.js 版本要求
...
| Next.js App Router route handlers | 支持 | Alpha 放在 server-only module，并使用 Node.js runtime。 |
...
| Cloudflare Workers 直接运行 Alpha SDK | 不推荐 | 使用 Cloudflare 原生 x402 和 Agents 支持，或调用运行 Alpha 的 Node 服务。 |
...
`runtime = "nodejs"` 的 Next.js server routes 中部署 Alpha。
